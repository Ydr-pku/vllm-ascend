...
def test_dyntra_lb_pause_emits_diagnostics(capsys):
    scheduler = _create_scheduler_with_diagnostics(True)
    request = create_request(request_id=1)
    scheduler.add_request(request)
    scheduler.schedule()
    ...
    assert f"DYNTRA_LB_PAUSE request_id={request.request_id}" in capsys.readouterr().out
    assert request in scheduler.waiting
...
def test_print_scheduler_summary_uses_effective_attention_block_size(capsys):
    scheduler = SimpleNamespace(
        running=[
            _diagnostic_request(
                "running",
