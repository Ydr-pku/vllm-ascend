...
def print_scheduler_summary(scheduler: Any, scheduler_output: Any) -> None:
    ...
    print(
        ...
        "scheduler req num: "
        f"[{len(scheduler.running)}, "
        f"{len(waiting_reqs)}, {waiting_num}, "
        f"{waiting_for_remote_kvs_num}, {waiting_for_fsm_num}, "
        f"{preempted_num}] | blk num "
        f"[{running_block_num}, {waiting_queue_block_num}, "
        f"{waiting_block_num}] | block size "
        f"[scheduler={scheduler.block_size}, "
        f"dyntra_lb={get_dyntra_lb_block_size(scheduler)}]",
        flush=True,
    )
...
class DyntraLBPolicyMixin(_SchedulerBase):
    ...
    def _lb_pause_request(
        ...
        if self._enable_diagnostics:
            print(
                f"DYNTRA_LB_PAUSE request_id={request.request_id}",
                flush=True,
            )
        if self.log_stats:
...
class DyntraLBScheduler(DyntraLBPolicyMixin, Scheduler):
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        # Apply cross-rank load-balancing decisions before allocating this step.
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                # Admit only requests selected by the load-balancing plan.
