...
- the vLLM V1 engine must be used
...
`--additional-config`. Top-level `DYNTRA_LB_*` keys are unsupported and do not
enable this feature.
...
| `enable_diagnostics` | bool | `false` | Prints DyntraLB configuration, per-rank workload snapshots, scheduling modifications, and scheduler summaries. |
| `mode` | str | `"dynamic"` | Selects `"static"` or `"dynamic"` activation. Use `"static"` only for troubleshooting; use `"dynamic"` for normal workloads. |
...
DyntraLB behavior easier to observe. Do not use static mode for normal workloads.
